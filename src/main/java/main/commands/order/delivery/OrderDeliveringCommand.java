...
public class OrderDeliveringCommand implements Command {
    @Override
    public String execute(HttpServletRequest request) {
        ...
        order.setStateId(OrderState.DELIVERING.ordinal());
        order.setWorkerId(userId);
        OrderDAO.getInstance().updateOrder(order);
        return controller + CommandName.COMMAND__SHOW_DELIVERY_ORDERS;
    }
